...
class SparseMoePrefetcher(nn.Module):
    def __init__(self, config, layer_id, gate, expert_cache, compute_stream):
        super().__init__()
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        expert_layer_list, expert_idx_list = self.experts.load_next_layer_expert(*((self.layer_id, expert_idx) for expert_idx in active_experts), layer_id=self.layer_id+1)
        while len(expert_layer_list) != 0:
            layer_id, expert_idx = expert_idx_list.popleft()
            # print("Expert id ", expert_idx)
            expert_layer = expert_layer_list.popleft()
